...
<div class="d-grid gap-2">
  <button class="btn btn-info" id="loadmorebtn" type="button">Button</button>
...
<script>
  $(document).ready(function() {
      ...
      $(document).on('click','.newcomment',function(){
        ...
        let newcommentuserid = @json(auth()->user()->id);
        let newcommentusername = @json(auth()->user()->name);
        $.ajax({
            url: "{{ route('addcomment')}}",
            type:"POST",
            data:{
                post_id: newcommentpostID,
                user_id: newcommentuserid,
                comment: newcomment,
            ...
            success: function(commentscount) {
              ...
              loadmoretag += '<img src="{{setImage(auth()->user()->profile_img, "user")}}" alt="avatar" class="rounded-circle me-2"style=" width: 38px; height: 38px; object-fit: cover; " />'
              loadmoretag += '<div class="p-3 rounded comment__input w-100">'
              loadmoretag += '<p class="fw-bold m-0">'+newcommentusername+'</p>'
              loadmoretag += '<p class="m-0 fs-7 p-2 rounded"> '+newcomment+' </p>'
              loadmoretag += '</div>'
              loadmoretag += '</div>'
              $('#comments_list_'+newcommentpostID).append(loadmoretag);
              $('#post_comment_'+newcommentpostID).text(commentscount+' comments');
    ...
    function postAdd(posts) { 
      ...
      posts.map(post => {        
            ...
            loadmoretag +='<img src="'+post.user.postprofile_img+'" alt="avatar" class="rounded-circle me-2" style="width: 38px; height: 38px; object-fit: cover" />'
            loadmoretag += '<div>'
            ...
            loadmoretag += '<div>';
            loadmoretag += (post.description) ? '<p>' +post.description+ '</p>':'';
            ...
            loadmoretag += '<ul class="reactions-box">'
            loadmoretag += '<a id="likebtn" data-like-type="{{ App\Models\Like::LIKE }}" data-post-id="'+post.id+'">'
            loadmoretag += '<li class="reaction reaction-like" data-reaction="Like"></li>'
            loadmoretag += '</a>'
            loadmoretag += '<a id="likebtn" data-like-type="{{ App\Models\Like::LOVE }}" data-post-id="'+post.id+'">'
            loadmoretag += '<li class="reaction reaction-love" data-reaction="Love"></li>'
            loadmoretag += '</a>'
            loadmoretag += '<a id="likebtn" data-like-type="{{ App\Models\Like::HAHA }}" data-post-id="'+post.id+'">'
            loadmoretag += '<li class="reaction reaction-haha" data-reaction="HaHa"></li>'
            loadmoretag += '</a>'
            loadmoretag += '<a id="likebtn" data-like-type="{{ App\Models\Like::WOW }}" data-post-id="'+post.id+'">'
            loadmoretag += '<li class="reaction reaction-wow" data-reaction="Wow"></li>'
            loadmoretag += '</a>'
            loadmoretag += '<a id="likebtn" data-like-type="{{ App\Models\Like::SAD }}" data-post-id="'+post.id+'">'
            loadmoretag += '<li class="reaction reaction-sad" data-reaction="Sad"></li>'
            loadmoretag += '</a>'
            loadmoretag += '<a id="likebtn" data-like-type="{{ App\Models\Like::ANGRY }}" data-post-id="'+post.id+'">'
            ...
            loadmoretag += '</i>'
            loadmoretag += '<a id="showcommens" class="showcomments" data-toggle="collapse" data-target="#post_comments_'+post.id+'">'
            ...
            loadmoretag += '<img src="{{setImage(auth()->user()->profile_img, "user")}}" alt="avatar" class="rounded-circle me-2" style=" width: 38px; height: 38px; object-fit: cover; " />'
            loadmoretag += '</div>'
            // loadmoretag += '<input type="hidden" name="post_id" id="InputpostId" value="'+post.id+'">'
    ...
    function commentshow(comments, postID) { 
          ...
          loadmoretag += '<img src="'+comment.user.commentprofile_img+'" alt="avatar" class="rounded-circle me-2"style=" width: 38px; height: 38px; object-fit: cover; " />'
          loadmoretag += '<div class="p-3 rounded comment__input w-100">'
          loadmoretag += '<p class="fw-bold m-0">'+comment.user.name+'</p>'
